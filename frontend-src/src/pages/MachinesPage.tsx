...
export default function MachinesPage() {
  ...
  return (
    <div
      className="h-full bg-[#09090b] text-[#e8e6f0] overflow-auto p-5"
      style={{
        backgroundImage:
    ...
    >
      ...
      <div className="flex items-end justify-between mb-5">
        <div>
          <div className="text-[11px] text-[#4ecdc4] uppercase tracking-widest mb-1">infrastructure</div>
          <div className="text-[32px] leading-none border-b-[3px] border-[#c0392b] pb-1">machines</div>
        </div>
        <button
          onClick={handleConnect}
          disabled={connecting}
          className="border border-white/[0.08] rounded bg-[#c0392b] text-black px-5 py-2 text-[13px] uppercase hover:bg-[#e04050] disabled:opacity-50"
      ...
      {connectResult && (
        ...
        >
          <div className="border-b border-white/[0.08] px-4 py-2 bg-[#0f1a18] flex items-center justify-between">
            <div className="text-[13px] uppercase text-[#4ecdc4]">connect machine</div>
            <button onClick={() => setConnectResult(null)} className="text-[#4a4a60] hover:text-[#e8e6f0] text-[18px] leading-none">×</button>
          </div>
          <div className="px-4 py-3 space-y-3">
            <div className="text-[12px] text-[#9a8888]">在目标机器上运行：</div>
            <div
              className="border border-white/[0.08] rounded bg-[#09090b] text-[#4ecdc4] px-4 py-3 text-[13px] font-mono cursor-pointer hover:bg-[#161218] break-all"
              onClick={() => navigator.clipboard.writeText(connectResult.connect_command)}
              title="点击复制"
            >
              {connectResult.connect_command}
            </div>
            <div className="text-[11px] text-[#4a4a60]">
      ...
      {machines.map((machine, i) => {
        ...
        return (
          <div
            key={machine.id}
            className="mb-5 border border-white/[0.08] rounded bg-[#191619]"
            style={{
          ...
          >
            {/* Machine header */}
            <div className="border-b border-white/[0.08] px-5 py-3 bg-[#1e1a20] flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div
                  className="w-10 h-10 border border-white/[0.08] rounded flex items-center justify-center text-[14px]"
                  style={{ background: isOnline ? '#0f1a18' : '#1e1a20', color: isOnline ? '#4ecdc4' : '#4a4a60' }}
                >
                  ⬡
                </div>
                <div>
                  {renamingId === machine.id ? (
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        className="border-[2px] border-[#4ecdc4] bg-[#09090b] text-[#e8e6f0] px-2 py-0.5 text-[14px] outline-none w-36"
                      />
                      <button onClick={() => handleRename(machine.id)} className="text-[#4ecdc4] text-[11px] hover:text-[#4ed0b0]">ok</button>
                      <button onClick={() => setRenamingId(null)} className="text-[#4a4a60] text-[11px]">✕</button>
                    </div>
                  ) : (
                    <div
                      className="text-[16px] cursor-pointer hover:text-[#4ecdc4] group flex items-center gap-1"
                      onClick={() => { setRenamingId(machine.id); setRenameValue(machine.name) }}
                      title="Click to rename"
                    >
                      {machine.hostname ?? machine.name}
                      <span className="text-[10px] text-[#4a4a60] opacity-0 group-hover:opacity-100">✎</span>
                    </div>
                  )}
                  <div className="flex items-center gap-2">
                    <span
                      ...
                      style={{
                        background: isOnline ? '#4ecdc4' : '#4a4a60',
                        animation: isOnline ? 'pulse 1.2s ease-in-out infinite' : 'none',
                      }}
                    />
                    <span className="text-[11px] uppercase" style={{ color: isOnline ? '#4ecdc4' : '#4a4a60' }}>
                      {machine.status}
                    </span>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="border border-white/[0.08] rounded bg-[#4ecdc4] text-black px-4 py-1 text-[12px] uppercase hover:bg-[#4ed0b0]"
                >
                  reconnect machine
                </button>
                <button
                  onClick={() => setDeleteConfirm(machine.id)}
                  className="border border-white/[0.08] rounded bg-[#1e1a20] text-[#4a4a60] px-3 py-1 text-[12px] uppercase hover:bg-[#2a2228] hover:text-[#c0392b]"
                >
                  remove
                </button>
              </div>
            ...
            {/* Machine meta */}
            <div className="px-5 py-3 grid grid-cols-4 gap-4 border-b border-white/[0.08] bg-[#0f0f14]">
            ...
            <div className="px-5 py-3">
              <div className="flex items-center justify-between mb-3">
                <div className="text-[11px] text-[#4a4a60] uppercase">
                ...
                <button
                  ...
                  className="border border-white/[0.08] bg-[#1a2535] text-[#4ecdc4] text-[11px] px-3 py-1 uppercase hover:bg-[#243548]"
                >
                  + new agent
                </button>
              </div>
              ...
              {agents.length === 0 ? (
                <div className="text-[12px] text-[#4a4a60] text-center py-4 border-[2px] border-dashed border-[#2a2228]">
                ...
                <div className="space-y-2">
                  {agents.map(agent => {
                    ...
                    return (
                      <div key={agent.id} className="flex items-center gap-3 border border-white/[0.08] bg-[#1e1a20] px-3 py-2">
                        <span
                          className="w-3 h-3 border border-black shrink-0"
                          style={{
                            background: isRunning ? '#4ecdc4' : '#3a3535',
                            animation: isRunning ? 'pulse 1.2s ease-in-out infinite' : 'none',
                          }}
                        />
                        <div className="flex-1 min-w-0">
                          <span className="text-[13px]">{agent.name}</span>
                          <span className="text-[11px] text-[#4ecdc4] ml-2">{agent.model_id}</span>
                        </div>
                        <span className="text-[10px] uppercase px-1.5 py-0.5 border border-black" style={{
                          ...
                          color:      agent.runtime === 'codex' ? '#7ecf50' : agent.runtime === 'kimi' ? '#a07ef0' : '#4ecdc4',
                        }}>
                          {agent.runtime ?? 'claude'}
                        </span>
                        <span className="text-[11px] uppercase" style={{ color: isRunning ? '#4ecdc4' : '#4a4a60' }}>
                          {agent.status}
                        </span>
                      </div>
                    )
          ...
          footer={(
            <div className="grid grid-cols-2">
              ...
              <button
                ...
                className="py-3 text-[12px] uppercase bg-[#4ecdc4] text-black hover:bg-[#4ed0b0] disabled:opacity-40"
              >
          ...
          <div className="space-y-3">
            <div>
              <div className="flex items-center justify-between gap-3 mb-1">
                <div className="text-[11px] text-[#4a4a60] uppercase">agent name *</div>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-3 gap-2">
                {(['claude', 'codex', 'kimi'] as const).map(rt => (
                  <button
                    ...
                    className="border border-white/[0.08] rounded py-2 text-[12px] uppercase transition-colors"
                    style={{
                      background: createAgentRuntime === rt
                        ? (rt === 'codex' ? '#1a2010' : rt === 'kimi' ? '#1a1535' : '#0f1a18')
                        : '#1e1a20',
                      color: createAgentRuntime === rt
                        ? (rt === 'codex' ? '#7ecf50' : rt === 'kimi' ? '#a07ef0' : '#4ecdc4')
                        : '#4a4a60',
                      borderColor: 'black',
            ...
            <div>
              ...
              <select
                ...
                className="rsl-control rsl-select w-full border border-white/[0.08] rounded bg-[#09090b] text-[#e8e6f0] px-3 py-2 text-[13px] outline-none placeholder:text-[#4a4a60]"
              >
        ...
        <DialogShell
          ...
          footer={(
            <button
              ...
              className="w-full py-3 text-[12px] uppercase text-[#4ecdc4] hover:bg-[#0f1a18]"
            >
              done
            </button>
          )}
      ...
      {deleteConfirm && (
        ...
        >
          <div className="text-[14px] text-[#e8e6f0] mb-2">确定要移除这台机器吗？</div>
          <div className="text-[11px] text-[#4a4a60]">关联的 Agent 不会被删除，但会失去机器绑定。</div>
        </DialogShell>
      )}
      ...
      {machines.length === 0 && (
        <div className="text-center py-16">
          ...
          <div className="text-[14px] text-[#4a4a60] mb-2">no machines connected</div>
...
function MetaItem({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div className="text-[10px] text-[#4a4a60] uppercase mb-1">{label}</div>
      <div className="text-[13px] text-[#8a8aa8] truncate">{value}</div>
    </div>
  )
}
