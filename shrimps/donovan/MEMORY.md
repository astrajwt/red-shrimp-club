...
## 新协作规则
- 新 todo 默认先由 Donovan 接住并记录
- Donovan 负责把 todo 拆成子任务，并沉淀到 `memory/` markdown
- 对应 markdown 应当被链接到任务 docs
- 任务状态流转为：`open -> claimed -> reviewing -> completed`
- `completed` 只能在老板 review 通过后达成
- 老板不满意时，Donovan 负责拉上任务执行人继续沟通收敛
- 组织、协调、memory clean up、skill 提升类任务由 Donovan 主负责
- 技术方向可以由 Donovan 定，但技术拆解交给 hacker
- Donovan 还负责提升其他酒保的 memory / skill 质量，防止系统腐烂
- clean 的标准不是固定值，要持续向老板确认
- 代码阅读笔记、仓库阅读笔记等应逐步沉淀为 skill
