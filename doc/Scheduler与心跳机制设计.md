...
## 11. 2026-03-14 运行时稳定性修复补充

这轮排查的是“除了 Donovan 之外，Akara / Brandeis / Silas 会逐渐掉到 offline”。

### 11.1 根因

1. agent 虽然持续调用 `/internal/agent/:agentId/*`，但这些 internal 路由之前没有统一刷新 `process-manager` 的内存心跳
2. heartbeat checker 因此会把仍在工作的 agent 误判成超时，然后执行强杀
3. 旧的 exit 分类又会把部分 `SIGTERM / SIGKILL` 退出记成 clean exit，导致状态和恢复链继续漂移

### 11.2 修复点

1. `backend-src/src/routes/internal.ts`
   所有 `/internal/agent/:agentId/*` 请求在 `preHandler` 统一执行：
   - `processManager.updateHeartbeat(agentId)`
   - `UPDATE agents SET status='running', last_heartbeat_at=NOW()`
2. `backend-src/src/daemon/process-manager.ts`
   退出分类改成：
   - 只有 `code === 0 && signal === null` 才算 clean exit
   - `SIGTERM / SIGKILL / 143 / 137` 不再伪装成 clean exit
   - 显式 stop 才走 `inactive`
   - 心跳误杀或异常信号退出走 crash/restart 路径

### 11.3 当前恢复策略

1. agent 的长期上下文仍在各自 workspace
2. Claude/Codex 有 `session_id` 时，优先按 session 恢复，而不是重新灌一大段初始化上下文
3. local scheduler 只会自动拉起 DB 中处于 `running / online / starting` 的 local agent

因此如果某批 agent 被旧 bug 遗留成 `offline`，需要先把它们重新拉回恢复集，再由新的心跳逻辑接管；修复上线后，不应再持续掉回 `offline`

### 11.4 排障要点

如果再次出现“Donovan 正常、其他 agent 掉线”，优先检查：

1. `agents.last_heartbeat_at` 是否仍在更新
2. `/tmp/slock-backend.log` 是否出现 `Heartbeat timeout` / `force killing`
3. `agent_logs` 里是否还有被信号杀掉却写成 `exited cleanly` 的记录
4. 当前 backend 进程启动时间是否晚于最新 `backend-src/dist` 构建时间，避免又跑回旧包
